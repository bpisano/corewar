...
t_op	*struct_tab2(t_op *op_tab)
{
	...
	op_tab[14] = (t_op){"lfork", 1, {T_DIR, 0, 0}, 15, 0, 2, 1};
	op_tab[15] = (t_op){"aff", 1, {T_REG, 0, 0}, 16, 1, 0, 1};
	return (op_tab);
}
...
t_op	*struct_tab(void)
{
	...
	T_DIR, T_REG}, 7, 1, 4, 1};
	op_tab[7] = (t_op){"xor", 3,
	{T_REG | T_IND | T_DIR, T_REG | T_IND | T_DIR, T_REG}, 8, 1, 4, 1};
	op_tab[8] = (t_op){"zjmp", 1, {T_DIR, 0, 0}, 9, 0, 2, 1};
	op_tab[9] = (t_op){"ldi", 3,
